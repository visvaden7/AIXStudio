    ...
    <CoverModal isOpen={isOpen} onClose={onClose} className={'w-[25%] mb-[25%] mr-[15%]'}>
      <div className={'flex flex-col gap-1 text-left'}>
        ...
        <div className={'flex gap-2 items-center p-4'}>
          <input type={'text'} placeholder={'내용을 입력해주세요'} className={'w-full border-2 border-[#C7C5BD] rounded-3xl p-2'}/>
          <div className={'flex justify-center items-center p-1 w-10 h-10 bg-[#1A2948] rounded-full'}><img src={sendImg} alt={'send-button'} className={''}/></div>
        </div>
      </div>
    </CoverModal>
  )
